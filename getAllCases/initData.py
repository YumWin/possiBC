...
with open('./source_file/sample.json','r',encoding='utf8')as fp:
    ...
    for key in json_data:
        ...
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id']))
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\字符串')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\线性表')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\数组')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\查找算法')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\排序算法')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\数字操作')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\树结构')
        os.makedirs('E:\\Sample\\'+str(json_data[key]['user_id'])+'\\图结构')
        for case in caseList:
            Fileinfo.append([])
            Fileinfo[i].append(str(json_data[key]['user_id']))
            Fileinfo[i].append(str(case['case_id']))
            Fileinfo[i].append(str(case['case_type']))
            Fileinfo[i].append(str(case['case_zip']))
            for record in case['upload_records']:
                Fileinfo[i].append(str(record['code_url']))
            i+=1

        # print(key+':'+str(json_data[key]))

# os.makedirs('E:\\SampleTest\\可')
download_and_extract(Fileinfo,'E:\\Sample')